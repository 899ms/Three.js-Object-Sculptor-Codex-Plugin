...
Hidden-form planning also defaults to one ImageGen 2x2 turnaround (`three-quarter | side` over `back | front`). Do not replace it with sequential generations: the shared sheet keeps identity, scale, and styling more consistent across views. It may be skipped only when the object is assessed as both simple and strongly symmetric; uncertainty means generate it.

Every new evidence view records `referenceProvenance`. Direct user/source references use `origin: observed`; a contract-approved white-background primary reference uses `origin: prepared-reference`; both use `allowedUse: acceptance`. Registered ImageGen 2x2 turnaround tiles use `origin: synthetic-hypothesis` and `allowedUse: planning-veto`; they may veto cross-view silhouette/depth failures but their inferred material appearance is not truth. A prepared primary reference approves reconstruction detail only while the retained original vetoes identity or macro-form drift. Turnaround cells never approve.
