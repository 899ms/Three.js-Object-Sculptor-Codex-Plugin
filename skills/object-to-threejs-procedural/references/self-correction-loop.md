...
Treat every valid attempt as a challenger to the best checkpoint in the active strategy. Promotion requires composite AI similarity `>=0.70` and blind-scout `approve`; explicit user approval closes the phase. The blind scout compares source/current/previous images to decide visual regression. Pixel-overlap diagnostics never participate.
